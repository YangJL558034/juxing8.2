  ...
  return (
    <div className={cn('space-y-4 pb-20', standaloneRequest && 'space-y-2 text-sm')}>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto grid w-full max-w-md grid-cols-3 border-t border-slate-200 bg-white/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur">
        {[['首页', '⌂'], ['我的领用', '▦'], ['我的申请', '♙']].map(([label, icon], index) => <button key={label} type="button" onClick={() => { setAnimatingNav(index); setActiveNav(index === 1 ? 'claims' : index === 2 ? 'apply' : 'home'); window.setTimeout(() => setAnimatingNav(null), 180); }} className={cn('flex flex-col items-center gap-1 py-1 text-[10px] transition-transform duration-150', animatingNav === index && 'scale-90', ((index === 0 && activeNav === 'home') || (index === 1 && activeNav === 'claims') || (index === 2 && activeNav === 'apply')) ? 'font-semibold text-blue-600' : 'text-slate-500')}><span className="text-lg leading-5">{icon}</span>{label}</button>)}
      </nav>

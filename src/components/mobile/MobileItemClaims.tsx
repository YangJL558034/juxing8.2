...
export default function MobileItemClaims({ canManage, standaloneRequest = false, initialApplicantName = '' }: MobileItemClaimsProps) {
  ...
  const [activeNav, setActiveNav] = useState<'home' | 'claims' | 'apply' | 'approved'>(canManage && !standaloneRequest ? 'claims' : 'home');
    ...
    <div className={cn('space-y-4 pb-20', standaloneRequest && 'space-y-2 text-sm')}>
      <section className={cn('mobile-ios-glass rounded-[30px] p-5 text-slate-950', standaloneRequest && 'rounded-[22px] p-3')}>
        ...
        <div className={cn('grid gap-2', showStockControls ? 'grid-cols-3' : 'grid-cols-1')}>
          ...
          {showStockControls && <Button variant="outline" className="h-12 rounded-2xl text-base font-semibold" onClick={() => { setActiveNav('approved'); setStockOpen(false); setClaimOpen(false); }}><Eye className="mr-2 h-4 w-4" />已领用记录</Button>}
      ...
      {
      <section id="item-claim-history" className={cn('space-y-3', activeNav === 'claims' || activeNav === 'apply' || activeNav === 'approved' ? '' : 'hidden')}>
        <div className="flex items-center justify-between px-1">
          <h2 className="text-base font-semibold text-slate-950">{activeNav === 'approved' ? '已领用记录' : activeNav === 'apply' ? '申请中的物品' : canManage ? '领用审核' : '我的领用'}</h2>
          <span className="text-sm text-slate-500">{claims.filter((claim) => activeNav === 'claims' ? (canManage ? claim.status === '待审核' : claim.status === '已审核') : activeNav === 'approved' ? claim.status === '已审核' : claim.status !== '已审核').length} 条</span>
        </div>
        ...
        {!loading && claims.filter((claim) => activeNav === 'claims' ? claim.status === '已审核' : claim.status !== '已审核').length === 0 && (
          ...
          {claims.filter((claim) => activeNav === 'claims' ? (canManage ? claim.status === '待审核' : claim.status === '已审核') : activeNav === 'approved' ? claim.status === '已审核' : claim.status !== '已审核').map((claim) => (
          <article
            key={claim.id}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto grid w-full max-w-md grid-cols-3 border-t border-slate-200 bg-white/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur">
        {[['首页', '⌂'], ['我的领用', '▦'], ['我的申请', '♙']].map(([label, icon], index) => <button key={label} type="button" onPointerDown={() => setAnimatingNav(index)} onPointerUp={() => { setActiveNav(index === 1 ? 'claims' : index === 2 ? 'apply' : 'home'); window.setTimeout(() => setAnimatingNav(null), 180); }} onPointerCancel={() => setAnimatingNav(null)} className={cn('flex flex-col items-center gap-1 py-1 text-[10px] transition-transform duration-150 active:scale-90', animatingNav === index && 'scale-90', ((index === 0 && activeNav === 'home') || (index === 1 && activeNav === 'claims') || (index === 2 && activeNav === 'apply')) ? 'font-semibold text-blue-600' : 'text-slate-500')}><span className="text-lg leading-5">{icon}</span>{label}</button>)}
      </nav>
